...
update ((delta, input) as ev) game =
  ...
  case (input, game.state) of
    ...
    -- if its a keypress and its the unpause button, then resume play
    (Pressed ks, Paused) ->
    if | (member 79 ks) -> { game | state <- Playing }
       | otherwise      -> game
    -- any other input remains in paused
    (_         , Paused) -> game

    -- if its a keypress
    (Pressed ks, Playing) -> game'
    _                     -> game'
...
sEvent : Signal Event
sEvent = Signal.mergeMany
         [ ((\t    -> (t, Tick))       <~ sDelta)
         , ((\t kp -> (t, Pressed kp)) <~ sDelta ~ K.keysDown)
         ]
